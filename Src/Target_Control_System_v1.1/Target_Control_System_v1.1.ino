...
void start_timer(void) {
  ...
  if (debug) Serial.println("Target timer has started -> targets unfaced");
}
...
void change_timer_phase(void) {
  ...
  switch (target_timer_phase) {
    ...
    case SHOOT_TIMER_PHASE:
      ...
      if (debug) Serial.println("Target timer SHOOT PHASE -> targets facing");
      break;
    ...
    case END_DELAY_PHASE:
      ...
      if (debug) Serial.println("Target timer END PHASE -> targets unfaced");
      break;
...
void cancel_timer(void) {
  ...
  if (debug) Serial.println("Target timer CANCELED -> targets facing");
}
...
void stop_timer(void) {
  ...
  leds.show();
  draw_timer(timer_val[timer_set_ptr]);
  if (debug) Serial.println("Target timer completed, STOP completed -> targets facing");
}
...
bool nrf_check(void) {
  ...
  if (radio.available(&pipe)) {              // is there a payload? get the pipe number that recieved it
    ...
    if (debug) {
      String data_info = (String)num_bytes + " bytes received on pipe " + (String)pipe + ", data: 0x";
    ...
    if (pairing && (data[0] == PAIR_BYTE)) {
      ...
      uint32_t remote_button = data[0];
      batt_level = data[1];
      tx_packet_num = data[2];  // Tx increments this number on every transmit so that consecutive packets are not identical, as these seem to be acknowledged and discarded by the Rx
      draw_remote();
      if (debug) {
        ...
        Serial.print("Got tx packet ID ");
        Serial.println(tx_packet_num);
      }
      if (screen_menu == HOME_SCREEN) {
